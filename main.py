...
class PleaForMercyPlugin(Star):
    ...
    async def on_group_ban_notice(self, event: AiocqhttpMessageEvent):
        ...
        try:
            ...
            if self.plea_mode == "llm":
                ...
                plea_message = await self._generate_llm_plea(
                    client, group_id, self_id, admin_name, group_name, duration_str
                )
            else:  # 默认使用 fixed 模式
    ...
    async def _generate_llm_plea(
        ...
        try:
            # 1. 获取群聊历史消息
            history_data = await client.get_group_msg_history(
                group_id=group_id, message_seq=0
            )
            ...
            llm_response = await provider.text_chat(
                prompt=prompt, contexts=contexts, system_prompt=self.llm_system_prompt
            )
